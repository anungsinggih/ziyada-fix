-- ============================================================
-- 0021_update_import_master_data_skip_duplicates.sql
-- Import Master Data: skip duplicate SKU, case-insensitive brand/category,
-- and prevent duplicate opening stock.
-- ============================================================
...
begin
    ...
    loop
        ...
        -- Skip total if SKU already exists
        select id into v_item_id from items where sku = v_item_sku limit 1;
        ...
        if v_brand_name <> '' then
            select id into v_brand_id from brands where lower(name) = lower(v_brand_name) limit 1;
        ...
        if v_category_name <> '' then
            select id into v_category_id from categories where lower(name) = lower(v_category_name) limit 1;
    ...
    return jsonb_build_object(
        ...
        'inserted_or_updated', v_inserted_count,
