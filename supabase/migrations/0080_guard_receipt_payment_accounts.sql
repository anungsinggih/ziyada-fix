-- ============================================================
-- 0080_guard_receipt_payment_accounts.sql
-- Ensure required COA accounts exist for AR/AP cash flows
-- ============================================================
...
begin
  ...
  if p_method is null or btrim(p_method) = '' then raise exception 'Payment method is required'; end if;
  ...
  if v_ar_acc_id is null then raise exception 'COA Codes missing (1200)'; end if;
...
begin
  ...
  if v_ap_acc_id is null then raise exception 'COA Codes missing (2100)'; end if;
  if p_amount <= 0 then raise exception 'Amount > 0'; end if;
