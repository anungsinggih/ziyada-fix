-- ============================================================
-- 0093_dashboard_cash_balance_include_banks.sql
-- Include bank accounts in dashboard cash balance
-- ============================================================
...
declare
  ...
  -- Finance Health
  v_total_ar numeric := 0; v_total_ap numeric := 0; v_cash_balance numeric := 0;
...
begin
  -- Sales & Inv
  select coalesce(sum(total_amount), 0), count(*) into v_sales_today, v_sales_count_today
  from public.sales where sales_date = current_date and status = 'POSTED';

  select coalesce(sum(total_amount), 0) into v_sales_month
  from public.sales where date_trunc('month', sales_date) = date_trunc('month', current_date) and status = 'POSTED';

  select coalesce(sum(total_amount), 0), count(*) into v_purchases_month, v_purchases_count_month
  from public.purchases where date_trunc('month', purchase_date) = date_trunc('month', current_date) and status = 'POSTED';

  select count(*) into v_total_items from public.items where is_active = true;
  select count(*) into v_low_stock_count
  from public.inventory_stock s
  join public.items i on s.item_id = i.id
  where i.is_active = true and s.qty_on_hand <= 5;
  ...
  select jsonb_agg(t) into v_recent_purchases from (
    ...
    from public.purchases p left join public.vendors v on p.vendor_id = v.id
    order by p.created_at desc limit 5
  ) t;

  -- Finance Health (Owner only)
  if v_is_owner then
    ...
    -- Cash + Bank (Kas, BCA, BRI)
    select array_agg(id) into v_cash_account_ids
    from public.accounts
    where code in ('1100', '1110', '1111');

    select (
      coalesce((select sum(debit - credit) from public.opening_balances where account_id = any(coalesce(v_cash_account_ids, '{}'))), 0) +
      coalesce((select sum(debit - credit) from public.journal_lines where account_id = any(coalesce(v_cash_account_ids, '{}'))), 0)
    ) into v_cash_balance;
  end if;
