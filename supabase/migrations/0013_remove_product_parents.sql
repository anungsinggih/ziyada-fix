-- Remove product_parents and parent_id usage (Items are core master data)

-- 1) Update import_master_data RPC to ignore parent fields
...
-- 2) Remove obsolete product parent RPC
drop function if exists public.rpc_save_product_complete(
    uuid,
    text,
    text,
    uuid,
...
-- 3) Drop product_parents and items.parent_id
alter table if exists public.items
    drop column if exists parent_id;

drop table if exists public.product_parents cascade;
