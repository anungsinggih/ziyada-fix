-- ============================================================
-- 0070_import_master_data_validate_type.sql
-- Validate item type strictly on import
-- ============================================================
...
begin
    ...
    loop
        ...
        -- Validate item type strictly
        v_item_type := upper(coalesce(row_data->>'type', 'FINISHED_GOOD'));
        if v_item_type not in ('FINISHED_GOOD','RAW_MATERIAL','TRADED') then
            raise exception 'Invalid item type: %', v_item_type;
        end if;
        ...
        v_min_stock := greatest(coalesce((row_data->>'min_stock')::numeric, 0), 0);
        v_initial_stock := greatest(coalesce((row_data->>'initial_stock')::numeric, 0), 0);
    ...
    return jsonb_build_object(
        ...
        'message', format('Processed %s rows, inserted %s, skipped %s.', v_processed_count, v_inserted_count, v_skipped_count)
    );
end;
$$;
