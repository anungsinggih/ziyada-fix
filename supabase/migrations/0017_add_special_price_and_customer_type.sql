-- Add special price tier and customer type

do $$ begin
  create type customer_type as enum ('UMUM', 'KHUSUS', 'CUSTOM');
exception when duplicate_object then null; end $$;

alter table if exists public.customers
  add column if not exists customer_type customer_type not null default 'UMUM';

alter table if exists public.items
  add column if not exists price_khusus numeric(14,2) not null default 0;
...
alter table if exists public.items
  ...
  check (price_default >= 0 and price_khusus >= 0 and default_price_buy >= 0);

-- Update import_master_data to accept price_khusus
...
begin
    ...
    loop
        ...
        -- Size (standardize by code or case-insensitive name)
        ...
        -- Color (standardize by code or case-insensitive name)
