-- ============================================================
-- 0085_import_master_data_purchase_rules.sql
-- Enforce purchase_price rules & update existing SKU cost only
-- ============================================================
...
declare
    ...
    v_existing_item_type item_type;
begin
    ...
    loop
        ...
        -- If SKU exists, only update purchase_price by rule
        select id, type into v_existing_item_id, v_existing_item_type
        from public.items
        ...
        if v_existing_item_id is not null then
            ...
            v_item_type := v_existing_item_type::text;

            if v_item_type = 'FINISHED_GOOD' then
                v_def_price_buy := 0;
            elsif v_item_type in ('RAW_MATERIAL','TRADED') then
                if v_def_price_buy <= 0 then
            ...
            update public.items
            set default_price_buy = v_def_price_buy,
                updated_at = now()
